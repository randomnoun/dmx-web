...
// import com.randomnoun.common.webapp.upload.DwrProgressMonitor;
import com.randomnoun.common.webapp.upload.FileProgressTO;
import com.randomnoun.common.webapp.upload.MultipartRequestProgressListener;
import com.randomnoun.common.webapp.upload.MonitoredDiskFileItemFactory;
import com.randomnoun.common.webapp.upload.ProgressTO;
import com.randomnoun.dmx.config.AppConfig;


 /**
  * An exact copy of the ExtendedMultiPartRequestHandler in randomnoun commons,
  * but removing the DWR reference.
  * 
...
public class ExtendedMultiPartRequestHandler 
  // use 'extends' instead of 'implements' to remove eclipse warning in struts-config.xml   
...

 {
    ...
    // @TODO maybe shove this into a Constants class
    public static final String PROGRESS_SESSION_KEY = "com.randomnoun.dmx.web.fileProgress";
     ...
     */
    protected static final Logger logger = Logger.getLogger(ExtendedMultiPartRequestHandler.class);
    ...
    public void handleRequest(HttpServletRequest request)
        ...
        // Get the struts config for the current request.
    	...
    	File tempDir = new File(appConfig.getProperty("webapp.fileUpload.tempDir"));
    	
    	logger.debug("Handling the request (maxFilesize=" + maxFilesize + ", diskThreshold=" + diskThreshold + ", tempDir=" + tempDir + ") ...");
        FileProgressTO progress = new FileProgressTO(0, request.getContentLength(), 0, 0, ProgressTO.STATUS_START);
        // request.getSession().setAttribute(DwrProgressMonitor.PROGRESS_SESSION_KEY, progress);
        request.getSession().setAttribute(PROGRESS_SESSION_KEY, progress);
        MultipartRequestProgressListener listener = new MultipartRequestProgressListener(progress);
		
        logger.debug("uploading file with monitoring in session '" + request.getSession().getId() + "' ..");
        // Create a factory for disk-based file items
        FileItemFactory factory = new MonitoredDiskFileItemFactory((int) diskThreshold, tempDir, listener);
       ...
       //----------------------------------------------------------------
        ...
        try {
        	...
        	logger.error("Something else entirely", e);
        }
    ...
    */
    ...
    protected String getRepositoryPath(ModuleConfig mc) {
        ...
        if (logger.isDebugEnabled()) {
			logger.debug("File upload temp dir: " + tempDir);
        }
